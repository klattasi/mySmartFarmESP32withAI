...
static PzemData readResponse() {
  ...
  while (idx < PZEM_RSP_LEN) {
    if (millis() - t0 > PZEM_TIMEOUT_MS) break;
    if (Serial.available()) rsp[idx++] = Serial.read();
  }

  if (idx < PZEM_RSP_LEN)                          return d;  // Timeout
  if (rsp[0] != PZEM_ADDR || rsp[1] != PZEM_FN_READ) return d;  // Wrong frame

  // Verify CRC (last 2 bytes, little-endian)
  uint16_t recvCRC = rsp[PZEM_RSP_LEN - 2] |
                     ((uint16_t)rsp[PZEM_RSP_LEN - 1] << 8);
  if (crc16(rsp, PZEM_RSP_LEN - 2) != recvCRC)    return d;  // CRC error
...
void setup() {
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, OLED_ADDR)) {
    while (true) delay(1000);  // Halt — OLED required
  }
  showSplash();
